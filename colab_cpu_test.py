...
# -*- coding: utf-8 -*-
"""
Colab CPU版OCR処理テスト（エンコーディング問題修正版）
"""
...
class ColabOCRProcessor:
    ...
    def process_pdf(self, pdf_path, max_pages=5):
        """
        PDFを処理してテキストを抽出（テスト用：最大5ページ）
        """
        try:
...
def main():
    ...
    メイン処理
    """
    print("=== Colab CPU版OCR処理テスト（エンコーディング修正版）===")
    ...
    for i, pdf_path in enumerate(test_files, 1):
        print(f"\n=== 処理中 ({i}/{len(test_files)}): {os.path.basename(pdf_path)} ===")
        
        try:
            # OCR処理（テスト用：最大5ページ）
            processor = ColabOCRProcessor()
            results = processor.process_pdf(pdf_path, max_pages=5)
    ...
    # 全結果を保存
    if all_results:
        output_path = "improved_ocr_test_results.json"
        with open(output_path, 'w', encoding='utf-8') as f:
            json.dump(all_results, f, ensure_ascii=False, indent=2)
        
        print(f"\n=== テスト処理完了 ===")
        print(f"総ページ数: {len(all_results)}")
        print(f"処理したPDF数: {len(test_files)}")
        ...
        print("\n=== テスト完了 ===")
        print("結果を確認して、品質が良ければ全ファイル処理に進みます")
    else:
        print("処理結果がありませんでした")
